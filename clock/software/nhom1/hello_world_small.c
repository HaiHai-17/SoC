...
const unsigned char SetMinuteArlam   = 0x00100;     // Switch 8   : Chinh phut bao thuc                   (1: Cho phep chinh phut bao thuc, 0 : Khong cho phep chinh phut bao thuc)
const unsigned char SetHourArlam     = 0x00200;     // Switch 9   : Chinh gio bao thuc                    (1: Cho phep chinh gio bao thuc , 0 : Khong cho phep chinh gio bao thuc)

const unsigned char SetDay           = 0x00800;     // Switch 11  : Chinh ngay 							  (1: Cho phep chinh ngay         , 0 : Khong cho phep chinh ngay)
const unsigned char SetMonth		 = 0x01000;     // Switch 12  : Chinh thang							  (1: Cho phep chinh thang		  , 0 : Khong cho phep chinh thang)
const unsigned char SetYear          = 0x02000;     // Switch 13  : Chinh nam							  (1: Cho pheo chinh nam		  , 0 : Khong cho phep chinh nam)
const unsigned char UpDownYear       = 0x04000;     // Switch 14  : Tang Giam Nam						  (1: Tang gia tri nam			  , 0 : Giam gia tri nam)

const unsigned char OnOFF_Arlam      = 0x20000;     // Switch 17  : Bat/Tat bao thuc                      (1: Bat bao thuc                , 0 : Tat bao thuc)
		...
		if(IORD(SWITCH_BASE,0) & control){
			if(IORD(SWITCH_BASE,0) & date_time){
				if(IORD(SWITCH_BASE, 0) & SetTime){
					...
					if(IORD(KEY_BASE,0)==0x07 && (IORD(SWITCH_BASE,0) & SetHourArlam )){
						while(IORD(KEY_BASE,0)==0x07);
						hh_SA++;
						if(hh_SA==24) hh_SA=0;
					}
				...
				if(IORD(KEY_BASE,0) == 0x0D && (IORD(SWITCH_BASE,0) & SetDay)){
					while(IORD(KEY_BASE,0)==0x0D);
					d++;
					if(d==32 && (m == 1 || m == 3 || m == 5 || m == 7 || m == 8 || m == 10 || m == 12))   d=1;
					else if(d== 31 && (m == 4 || m == 6 || m == 9 || m == 11))							  d=1;
					else if(m==2){
						if(d==30 && y % 4 == 0)  														  d=1;
				...
				if(IORD(KEY_BASE,0)==0x07 && (IORD(SWITCH_BASE,0) & SetYear )){
					while(IORD(KEY_BASE,0)==0x0B);
					if(IORD(SWITCH_BASE,0) & UpDownYear)    y++;
					else                                    y--;
					if(y<=2000) y = 2000;
				}
			}
